...
//! Internal Galois functionality - Use at your own risk.
    namespace runtime {
        ...
        protected:
            ...
            struct ThreadLocalData {
                ...
                explicit ThreadLocalData(uint32_t n) : facing(), facing2(n) {
                    for(uint32_t i=0;i<n;i++) { // seem not useful, the bottle neck is still memory bound
            ...
            void doScatter(auto* &p, auto& tld, bool &didWork){
                while (p) {
                    ...
                    p->clear();
                    while(!tld.coro_sca()) tld.coro_syn();
                    tld.tmp.clear();
                    p = wl.pop3();
                }
            }
                ...
                while(wl.pop_part()){
                    ...
                    while (pg) {
                        for (int i=0;i<pg->count;i++){
                            ...
                            else tld.high.emplace_back(it);
                        }
                        pg->clear();
                        while(!tld.coro_gat()){};
                        tld.low.clear(); tld.high.clear();
                        pg = wl.pop_gather();
                    }
                    if (!tld.facing.empty()) {
            ...
            Corobj<bool> coro_scatter(auto &ctx2, auto& tmp){
                ...
                for(;;){
                    for(int id = 0; id< (int)tmp.size(); id+=64) {
                        ...
                        co_yield false;
                        for (int prid = id; prid < std::min(id+64, (int)tmp.size()); prid++) { // scatter
                            auto &vtxA = graph.plgraph[(*tmp[prid]).vid];
                            auto *Arr = vtxA.PE;
                            if (vtxA.deg2) _mm_prefetch(&graph.pledge[vtxA.offset], _MM_HINT_T0);
                            for (uint32 e = 0; e < vtxA.deg1; e += 2) {
                                if (!(Arr[e] >> MAX_OFS)) ctx2[Arr[e] >> 18].emplace_back(Arr[e], Arr[e + 1], (*tmp[prid]).dist);
                                else ctx2[Arr[e] & DW_HALF].emplace_back(MAX_BIT | ((Arr[e] & MIN_HFUP)>>HF_OFST), Arr[e + 1], (*tmp[prid]).dist);
                            }
                            Arr = graph.pledge;
            ...
            Corobj<bool> coro_gather(auto &ctx, auto& low, auto& high){
                ...
                for(;;){
                    ...
                    for(int id = 0; id< (int)high.size(); id+=64){
                        for(int prid = id; prid<std::min(id+64, (int)high.size()); prid++){
        ...
        public:
            ...
            void initFrontier(Frontier &front){
                for(auto & a : front) {
                    wl.push(a);
                }
                front.clear();
        ...
        template <typename OBIM, typename Graph, typename Indexer, typename Func, typename Range>// RangeTy = LocalRange<InsertBag>
        void asyncPriorityEdgeMap(Graph &graph, const Indexer & indexer, Func func,const Range &range) {
            auto& barrier = getBarrier(activeThreads);
            PriorityExecutor<OBIM, Func, Indexer, Graph> W(func, indexer, graph);
            substrate::getThreadPool().run(activeThreads, [&W, &range]() {W.initThread(range());}, std::ref(barrier),std::ref(W));
//            W.print_profiling();
        }
        ...
        template <typename OBIM, typename Graph, typename Frontier, typename Indexer, typename Func>// RangeTy = LocalRange<InsertBag>
        void syncEdgeMap(Graph &graph, Frontier& frontier, const Indexer & indexer, Func func) {
            auto& barrier = getBarrier(activeThreads);
