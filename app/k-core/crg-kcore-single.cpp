...
struct KCORE_F{
    ...
    uint32 corenum;
    explicit KCORE_F(unsigned int* _curdeg, uint32 _cr) : vdata(_curdeg), corenum(_cr){}
    ...
    inline vw pushFunc(uint32 dst, uint32 newdis) const {
        return vw(dst, corenum - 1);
    }
    static inline unsigned int applyWeight (unsigned int updateVal, unsigned int weight){
        return 0;
    }
};
...
void init_galois(int argc, char** argv) {
    for (int i = 1; i < argc; i++){
        if (i + 1 != argc){
            if (strcmp(argv[i], "-corenum") == 0) { // k = ?
                corenum = (unsigned int)atoi(argv[i + 1]);
                i++;
            }
...
int main(int argc, char** argv) {
    ...
    std::cout << "INFO: Using k-core number of " << corenum << "\n";
